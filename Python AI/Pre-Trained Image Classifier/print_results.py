...
def print_final_result(model, results_stats_dic):
        ...
        print(
            format_str.format(
                model,
                '{:05.2f}%'.format(results_stats_dic['pct_correct_notdogs']),
                '{:05.2f}%'.format(results_stats_dic['pct_correct_dogs']),
                '{:05.2f}%'.format(results_stats_dic['pct_correct_breed']),
                '{:05.2f}%'.format(results_stats_dic['pct_match'])
            )
        )
        print('-'*80)
        print("\r\n")
...
def print_results(results_dic, results_stats_dic, model, print_incorrect_dogs=False, print_incorrect_breed=False):
    ...
    if results_dic is not None:
        ...
        for k, v in results_stats_dic.items():
            if str(k).startswith('pct_'):
                pct_keys.append('{0:<40} {1:000.2f}%'.format(k, v))
            else:
                num_keys.append('{0:<40} {1}'.format(k, v))

        for keys in num_keys:
            print(keys)

        print("\r\n** Percentage Calculations: **\r\n")
        for keys in pct_keys:
            print(keys)
        ...
        print("\r\n")
        print_final_result(model, results_stats_dic)
